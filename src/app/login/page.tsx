  ...
  async function absenden(e: React.FormEvent) {
    ...
    if (modus === "passwort") {
      ...
      const { error } = await supabase.auth.signInWithOtp({
        email,
        options: { emailRedirectTo: `${location.origin}/auth/callback?weiter=${encodeURIComponent(weiter)}` },
      });
      if (error) {
  ...
  return (
    <main className="flex min-h-dvh items-center justify-center bg-navy-900 px-4 py-10">
      <div className="w-full max-w-md">
        ...
        <div className="card">
          <span className="badge-pill">Patientenbereich</span>
          ...
          <p className="mt-1 text-navy-600/80">
            Melden Sie sich an, um Termine, Trainingsplan und Nachrichten zu sehen.
          </p>
